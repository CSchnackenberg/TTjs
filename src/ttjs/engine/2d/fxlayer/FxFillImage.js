define(["require", "exports", "@ttjs/engine/2d/fxlayer/FxLayer", "@ttjs/util/TTTools"], function (require, exports, FxLayer_1, TTTools_1) {
    ...
    exports.FxFillImage = FxFillImage;
    ;
    TTTools_1.TTTools.inherits(FxFillImage, FxLayer_1.FxLayer);
    ...
    FxFillImage.prototype._drawNoRepeat = function (fxWorld, vp, imgW, imgH) {
        fxWorld.ctx.drawImage(this.image, Math.floor(-vp.x), Math.floor(-vp.y), imgW, imgH);
    };
    FxFillImage.prototype._drawRepeatBoth = function (fxWorld, vp, imgW, imgH) {
        ...
        for (var y = 0; y < numToDrawY; y++)
            for (var x = 0; x < numToDrawX; x++) {
                fxWorld.ctx.drawImage(this.image, Math.floor(-ox + x * imgW), Math.floor(-oy + y * imgH), imgW, imgH);
                nd++;
            }
    };
    FxFillImage.prototype._drawRepeatX = function (fxWorld, vp, imgW, imgH) {
        ...
        var nd = 0;
        for (var x = 0; x < numToDrawX; x++) {
            fxWorld.ctx.drawImage(this.image, Math.floor(-ox + x * imgW), -vp.y, imgW, imgH);
            nd++;
        }
    };
    FxFillImage.prototype._drawRepeatY = function (fxWorld, vp, imgW, imgH) {
        ...
        var nd = 0;
        for (var y = 0; y < numToDrawY; y++) {
            fxWorld.ctx.drawImage(this.image, -vp.x, Math.floor(-oy + y * imgH), imgW, imgH);
            nd++;
        }
    };
});
// 	return FxFillImage;
// });
//# sourceMappingURL=FxFillImage.js.map