...
define(['ttjs/lib/lodash'], function(_) {
	 ...
	 * @class circle
	 */
	QuadTree.Circle = function(x, y, radius) {    
		this.x = x || 0;
		this.y = y || 0;	
		this.radius = radius || 0;
	};

	QuadTree.Circle.prototype = {
		left: function() {
	 ...
	 * @class rectangle
	 */
	QuadTree.Rect = function(x, y, w, h) {    
		this.x = x || 0;
		this.y = y || 0;	
		this.w = w || 0;
		this.h = h || 0;
	...
	QuadTree.Rect.prototype = {
		...
		getQuadrant: function(index) {
			switch(index) {
				case 0: return new QuadTree.Rect(this.x, this.y, this.w*0.5, this.h*0.5);
				case 1: return new QuadTree.Rect(this.x+this.w*0.5, this.y, this.w*0.5, this.h*0.5);
				case 2: return new QuadTree.Rect(this.x, this.y+this.h*0.5, this.w*0.5, this.h*0.5);
				case 3: return new QuadTree.Rect(this.x+this.w*0.5, this.y+this.h*0.5, this.w*0.5, this.h*0.5);
			}
		},
		top: function() {
			return this.y;
	...
	QuadTree.QuadTreeNode = function(tree, x, y, width, height) {
		this.tree = tree;
		/** sub quadrants of this node */
		this.quadrants = null;
		/** node elements */
		this.elements = [];
		if (x instanceof QuadTree.Rect)
			this.quad = x;
		else
			this.quad = new QuadTree.Rect(x, y, width, height);
	};

	QuadTree.QuadTreeNode.prototype = {         
		addElement: function(e, bounds) {						
			...
			if (!this.quadrants)
				this.quadrants = [
					new QuadTree.QuadTreeNode(this.tree, this.quad.getQuadrant(0)),
					new QuadTree.QuadTreeNode(this.tree, this.quad.getQuadrant(1)),
					new QuadTree.QuadTreeNode(this.tree, this.quad.getQuadrant(2)),
					new QuadTree.QuadTreeNode(this.tree, this.quad.getQuadrant(3)),
				];
...
//	QuadTree._hash_current = 0;
	...
	QuadTree.prototype = {		
		addElement: function(e) {			
			...
			if (!(bounds instanceof QuadTree.Rect ||
				  bounds instanceof QuadTree.Circle)) {
		...
		rebuild: function(newElements, saveBorder) {
			...
			if (len > 0) {
				...
				if (this.forceQuad) {
					...
					this._root = new QuadTree.QuadTreeNode(this, cx-newSize, cy-newSize, newSize*2, newSize*2);
				}
				else {
					var newW = (r-cx) + saveBorder;
					var newH = (b-cy) + saveBorder;
					this._root = new QuadTree.QuadTreeNode(this, cx-newW, cy-newH, newW*2, newH*2);	
				}
				var allOk = true;
		...
		queryElements: function(queryShape) {			
			if (!(queryShape instanceof QuadTree.Rect ||
				  queryShape instanceof QuadTree.Circle)) {
